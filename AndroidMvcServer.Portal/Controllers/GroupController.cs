    ...
    public class GroupController : Controller
    {
        ...
        public JsonResult getAllGroupsByDepID(string depId)
        {
            ...
            foreach (SimpleGroupsModel2 model in list)
            {
                ...
                //1.根据groupOwner获取所在部门
                string strSql = "SELECT DeptId FROM Tb_User WHERE UserId='" + groupOwner + "'";
                string sqlDep = MySqlHelper.getFirstRow(strSql);
                //2.根据groupOwner获取姓名
                string strSql2 = "SELECT UserName FROM Tb_User WHERE UserId='" + groupOwner + "'";
                string sqlName = MySqlHelper.getFirstRow(strSql2);
                model.owner = sqlName;
                model.leaf = true;
                if (sqlDep.Substring(0, 2) == depId)//如果部门是北京、武汉、西安
                {
                ...
                else if (sqlDep.Substring(0, 4) == depId)
                {
                    string sqlString = "UPDATE Tb_Group SET groupName = '" + groupName + "',currentUsers=" + currentUsers + " WHERE groupId = '" + groupId + "'";
